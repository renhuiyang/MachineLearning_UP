...
function getMetricInfo(host_name, callback) {
	$.getJSON("/monitor/total?host="+host_name, function(data) {
		callback(data);
	});
}
...
function drawCpuTotalUsage(elementId,metrics) {
	...
	for (var i = 1; i < metrics.length; i++) {
		...
		elements.push(cur.cpu_usage_percent);
		data.push(elements);
	}
...
function drawMemoryUsage(elementId,metrics) {
	var titles = ["Time", "Total"];
	var data = [];
	for (var i = 0; i < metrics.length; i++) {
		...
		var elements = [];
		elements.push(cur.time);
		elements.push(cur.memory_usage);
		data.push(elements);
	}
...
function drawNetworkBytes(elementId, metrics) {
	...
	for (var i = 1; i < metrics.length; i++) {
		...
		var elements = [];
		elements.push(cur.time);
		elements.push(cur.network_rx_rate);
		elements.push(cur.network_tx_rate);
		data.push(elements);
	}
...
function drawFileSystemUsage(elementId, metrics) {
    var titles = ["Time", "Disk"];
	var data = [];
	for (var i = 1; i < metrics.length; i++) {
		var cur = metrics[i];
		var elements = [];
		elements.push(cur.time);
		elements.push(cur.disk_usage);
		data.push(elements);
	}
	drawLineChart(titles, data, elementId, "Percentage");
}
...
function drawCharts(metrics) {
	...
	// CPU.
	steps.push(function() {
		drawCpuTotalUsage("cpu-total-usage-chart", metrics);
	});

	// Memory.
		steps.push(function() {
		drawMemoryUsage("memory-usage-chart", metrics);
	});

	// Network.
		steps.push(function() {
		drawNetworkBytes("network-bytes-chart", metrics);
	});
		
	// Filesystem.
		steps.push(function() {
		drawFileSystemUsage("filesystem-usage-chart", metrics);
	});
...
function refreshStats(host_name) {
	$.getJSON("/monitor/total?host="+host_name, function(data) {
		drawCharts(data);
	});
	//var metrics = window.monitor.metrics;
	//drawCharts(metrics);
}
...
function startPage(host_name) {
	window.charts = {};
	window.monitor = {};
	
	// Get machine info, then get the stats every 1s.
	getMetricInfo(host_name, function(data) {
		window.monitor.metrics = data;
		refreshStats(host_name);
